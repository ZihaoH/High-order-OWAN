...
class OperationLayer(nn.Module):
    def __init__(self, C, stride, rank):
        ...
        self.w_cp1 = torch.nn.Parameter(torch.Tensor(rank, C*self.op_num))
        self.w_cp2 = torch.nn.Parameter(torch.Tensor(rank, C*self.op_num, rank))
        self.w_cp3 = torch.nn.Parameter(torch.Tensor(rank, C*self.op_num, rank))
        self.w_cp4 = torch.nn.Parameter(torch.Tensor(rank, C*self.op_num, rank))

        self.w_out3 = torch.nn.Parameter(torch.randn(rank,C))

        with torch.no_grad():
            self.w_cp1.normal_(0, 1/(C*self.op_num))
            self.w_cp2.normal_(0, 1/(rank*C*self.op_num))
            self.w_cp3.normal_(0, 1/(rank*C*self.op_num))
            self.w_cp4.normal_(0, 1/(rank*C*self.op_num))
    ...
    def forward(self, x, weights):
        ...
        out1 = torch.einsum('achw,rc->arhw', (states, self.w_cp1))
        out2 = torch.einsum('achw,kcp->akhwp', (states, self.w_cp2))
        out3 = torch.einsum('achw,pcu->aphwu', (states, self.w_cp3))
        out4 = torch.einsum('achw,ucy->auhwy', (states, self.w_cp3))

        return self.softsign(self.norm(torch.einsum('arhw,akhwp,aphwu,auhwy,uo->aohw',(out1,out2,out3,out4,self.w_out3))))
