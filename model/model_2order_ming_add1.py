#!/usr/bin/env python
# -*- coding: utf-8 -*-
# 2 order, CP decomposition, add 1, share parameter
import torch
...
class OperationLayer(nn.Module):
    def __init__(self, C, stride, rank):
        ...
        self.w_cp1 = torch.nn.Parameter(torch.Tensor(rank, C*self.op_num+1, C))
    ...
    def forward(self, x, weights):
        ...
        if states.is_cuda:
            one = torch.ones(states.shape[0],1,states.shape[2],states.shape[3]).cuda()
        else:
            one = torch.ones(states.shape[0],1,states.shape[2],states.shape[3])
        states=torch.cat([states,one], dim=1)

        out = torch.einsum('achw,rco->arohw', (states, self.w_cp1))

        return self.softsign(self.norm(torch.einsum('arohw,arohw,r->aohw',(out,out,self.w_out3))))
