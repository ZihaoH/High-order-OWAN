...
class OperationLayer(nn.Module):
    def __init__(self, C, stride, rank):
        ...
        self.w_cp1 = torch.nn.Parameter(torch.Tensor(rank, C*self.op_num, C))
        self.w_cp2 = torch.nn.Parameter(torch.Tensor(rank, C*self.op_num, C))
        self.w_cp3 = torch.nn.Parameter(torch.Tensor(rank, C*self.op_num, C))
        self.w_cp4 = torch.nn.Parameter(torch.Tensor(rank, C*self.op_num, C))

        self.w_out3 = torch.nn.Parameter(torch.randn(rank))

        with torch.no_grad():
            self.w_cp1.normal_(0, 1/(C*self.op_num))
            self.w_cp2.normal_(0, 1/(C*self.op_num))
            self.w_cp3.normal_(0, 1/(C*self.op_num))
            self.w_cp4.normal_(0, 1/(C*self.op_num))
            self.w_out3.normal_(0, 1/rank)
    ...
    def forward(self, x, weights):
        ...
        out1 = torch.einsum('achw,rco->arohw', (states, self.w_cp1))
        out2 = torch.einsum('achw,rco->arohw', (states, self.w_cp2))
        out3 = torch.einsum('achw,rco->arohw', (states, self.w_cp3))
        out4 = torch.einsum('achw,rco->arohw', (states, self.w_cp3))

        return self.softsign(self.norm(torch.einsum('arohw,arohw,arohw,arohw,r->aohw',(out1,out2,out3,out4,self.w_out3))))
...
if __name__=='__main__':
    ...
    np.random.seed(2018)
    random_x=torch.Tensor(np.random.randn(32, 3,63, 63))
    random_y=random_x
    handle = pynvml.nvmlDeviceGetHandleByIndex(0)

    L1_loss = nn.L1Loss().cuda()
    model = Network(16, 3, L1_loss,steps=4,rank=24).cuda()
